...
# run this model with HSM_midday_P50_mean_scaled as a predictor instead of P50_scaled

mod3 <- lmer(internode_length_1_2~ diameter_complete_scaled + predawn_scaled + P50_scaled + canopy_closure_scaled + elevation_ft_scaled + (1|plot), data = ABCO)
summary(mod3)
...
mod4 <- lmer(predawn_MPa_combined ~ species + diameter_complete_scaled + canopy_closure_scaled + elevation_ft_scaled + (1|plot) + (1|year), data = trait_data1)
summary(mod4)

# run this model for the following response variables for table S5
mod5 <- lmer(predawn_MPa_combined ~ species * diameter_complete_scaled * elevation_ft_scaled * canopy_closure_scaled + (1|plot) + (1|year), data = trait_data1)
summary(mod5)

# table 5 pairwise comparisons using emmeans
emms1 <- emmeans(mod5, pairwise ~ species)
