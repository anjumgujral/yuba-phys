...
trait_data2 <- merge(trait_data1, climate_df, by.x = "plot", by.y="id")

# now test whether climate explains water potentials 
# run the following model with each of the 5 climate variables (ppt, def, vpd, aet, pet) individually for both predawn and midday water potentials as the repsonse variable 

trait_data2$def_scaled <- as.vector(scale(trait_data2$def_30yr_mean))
trait_data2$ppt_scaled <- as.vector(scale(trait_data2$ppt_30yr_mean))

mod8 <- lmer(midday_MPa_combined~ diameter_complete_scaled + canopy_closure_scaled + ppt_scaled + (1|plot) + (1|wp_year), data = trait_data2)
summary(mod8)
