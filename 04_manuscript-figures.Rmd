...
for (sp in names(species_list_elev)) {
  ...
  mean_elev <- mean(df$elevation_ft_scaled, na.rm = TRUE)
  sd_elev   <- sd(df$elevation_ft_scaled, na.rm = TRUE)

  # Sequence of elevations
  elev_seq <- seq(min(df$elevation_ft_scaled, na.rm = TRUE),
                  max(df$elevation_ft_scaled, na.rm = TRUE),
                  length.out = 100)
