...
```{r}
### FIGURE 1A: Predawn by size (partial residuals)

all_partial_resids <- data.frame()
model_summaries <- data.frame()
...
for (sp in names(species_list)) {
  ...
  # Fit species-specific model
  if (sp == "ABMA") {
    mod <- lmer(predawn_MPa_combined ~ diameter_complete_scaled + canopy_closure_scaled +
                  (1|plot) + (1|wp_year), data = df)
  } else {
    mod <- lmer(predawn_MPa_combined ~ diameter_complete_scaled + canopy_closure_scaled +
                  elevation_ft_scaled + (1|plot) + (1|wp_year), data = df)
  ...
  # Extract fixed-effect p-value for diameter (safe)
  summ <- summary(mod)$coefficients
  diam_p <- if ("diameter_complete_scaled" %in% rownames(summ) &&
                "Pr(>|t|)" %in% colnames(summ)) {
    summ["diameter_complete_scaled", "Pr(>|t|)"]
  } else {
    NA
  }
  ...
  model_summaries <- dplyr::bind_rows(
    model_summaries,
    data.frame(species = sp,
               p_value = diam_p,
               signif = signif_label)
  ...
  beta_diam <- if ("diameter_complete_scaled" %in% names(fixef(mod))) {
    fixef(mod)["diameter_complete_scaled"]
  } else {
    NA
  }

  df$partial_resid <- resid(mod) + beta_diam * df$diameter_complete_scaled
  df$species <- sp
  df$signif <- signif_label

  # Store
  all_partial_resids <- dplyr::bind_rows(all_partial_resids, df)
}
...
S2A <- ggplot() +
  ...
  geom_point(
    data = all_partial_resids,
    aes(x = diameter_complete,
        y = partial_resid,
        color = species),
    alpha = 0.2,
    size = 2,
