...
for (sp in names(species_list)) {
  ...
  # Fit species-specific model
  if (sp == "ABMA") {
    mod <- lmer(P50_combined ~ diameter_complete_scaled + canopy_closure_scaled + (1|plot) + (1|wp_year), data = df)
  } else {
    mod <- lmer(P50_combined ~ diameter_complete_scaled + canopy_closure_scaled + elevation_ft_scaled + (1|plot) + (1|wp_year), data = df)
  }
...
# Should be near zero — canopy closure effect has been removed
